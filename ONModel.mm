...
////////////////////////////////////////////////////////////////////////////////
//
//  RhinoViewer extensions to ONX_Model
//


void EX_ONX_Model::GetObjectMaterial( 
                                   int object_index,
                                   ...
                                   ) const
{
  ...
  {
    const EX_ONX_Model_Object& mo = m_object_table[object_index];
    if ( 0 != mo.m_object )
    {
...
ON_BOOL32 EX_ONX_Model::initWithFilename(const char* sFileName)
{
  DLog (@"EX_ONX_Model %p initWithFilename: %s", this, sFileName);

	// read the file
	ON_wString errString;
	ON_TextLog error_log (errString);
	if ( !Read( sFileName, &error_log ) )
	{
		// read failed
		return FALSE;
	}
...
void EX_ONX_Model::GetDefaultView( const ON_BoundingBox& bbox, ON_3dmView& view )
{
...
//
//  End of RhinoViewer extensions to ONX_Model
//  The rest is barely modified ONX_Model and friends
//
////////////////////////////////////////////////////////////////////////////////
...
//
//
#pragma message( " --- OpenNURBS overriding EX_ONX_Model new and delete" )

// EX_ONX_Model_UserData new/delete

void* EX_ONX_Model_UserData::operator new(size_t sz)
{
  // EX_ONX_Model_UserData new
  return onmalloc(sz);
}

void EX_ONX_Model_UserData::operator delete(void* p)
{
  // EX_ONX_Model_UserData delete
  onfree(p);
}

void* EX_ONX_Model_UserData::operator new[] (size_t sz)
{
  // EX_ONX_Model_UserData array new
  return onmalloc(sz);
}

void EX_ONX_Model_UserData::operator delete[] (void* p)
{
  // EX_ONX_Model_UserData array delete
  onfree(p);
}

void* EX_ONX_Model_UserData::operator new(size_t, void* p)
{
  // EX_ONX_Model_UserData placement new
  return p;
}

void EX_ONX_Model_UserData::operator delete(void*, void*)
{
  // EX_ONX_Model_UserData placement delete
  return;
}


// EX_ONX_Model_Object new/delete

void* EX_ONX_Model_Object::operator new(size_t sz)
{
  // EX_ONX_Model_Object new
  return onmalloc(sz);
}

void EX_ONX_Model_Object::operator delete(void* p)
{
  // EX_ONX_Model_Object delete
  onfree(p);
}

void* EX_ONX_Model_Object::operator new[] (size_t sz)
{
  // EX_ONX_Model_Object array new
  return onmalloc(sz);
}

void EX_ONX_Model_Object::operator delete[] (void* p)
{
  // EX_ONX_Model_Object array delete
  onfree(p);
}

void* EX_ONX_Model_Object::operator new(size_t, void* p)
{
  // EX_ONX_Model_Object placement new
  return p;
}

void EX_ONX_Model_Object::operator delete(void*, void*)
{
  // EX_ONX_Model_Object placement delete
  return;
}


// EX_ONX_Model_RenderLight new/delete

void* EX_ONX_Model_RenderLight::operator new(size_t sz)
{
  // EX_ONX_Model_RenderLight new
  return onmalloc(sz);
}

void EX_ONX_Model_RenderLight::operator delete(void* p)
{
  // EX_ONX_Model_RenderLight delete
  onfree(p);
}

void* EX_ONX_Model_RenderLight::operator new[] (size_t sz)
{
  // EX_ONX_Model_RenderLight array new
  return onmalloc(sz);
}

void EX_ONX_Model_RenderLight::operator delete[] (void* p)
{
  // EX_ONX_Model_RenderLight array delete
  onfree(p);
}

void* EX_ONX_Model_RenderLight::operator new(size_t, void* p)
{
  // EX_ONX_Model_RenderLight placement new
  return p;
}

void EX_ONX_Model_RenderLight::operator delete(void*, void*)
{
  // EX_ONX_Model_RenderLight placement delete
  return;
}


// EX_ONX_Model new/delete

void* EX_ONX_Model::operator new(size_t sz)
{
  // EX_ONX_Model new
  return onmalloc(sz);
}

void EX_ONX_Model::operator delete(void* p)
{
  // EX_ONX_Model delete
  onfree(p);
}
...
void* EX_ONX_Model::operator new[] (size_t sz)
{
  // EX_ONX_Model array new
  return onmalloc(sz);
}

void EX_ONX_Model::operator delete[] (void* p)
{
  // EX_ONX_Model array delete
  onfree(p);
}

void* EX_ONX_Model::operator new(size_t, void* p)
{
  // EX_ONX_Model placement new
  return p;
}

void EX_ONX_Model::operator delete(void*, void*)
{
  // EX_ONX_Model placement delete
  return;
}
...
static
bool EX_ONX_IsValidNameFirstChar( wchar_t c )
{
  if ( c > 127 )
    return true;
  if ( c < '0' )
...
static
bool EX_ONX_IsValidNameSecondChar( wchar_t c )
{
...
{
  bool is_valid = (0 != name && EX_ONX_IsValidNameFirstChar(*name));
  if ( is_valid )
  {
    bool is_integer = (*name >= '0' && *name <= '9');
    name++;
    while ( EX_ONX_IsValidNameSecondChar(*name) )
    {
      if ( *name < '0' || *name >= '9' )
        is_integer = false;
      name++;
...
EX_ONX_Model_RenderLight::EX_ONX_Model_RenderLight()
{
}
...
EX_ONX_Model_RenderLight::EX_ONX_Model_RenderLight(const EX_ONX_Model_RenderLight& src) 
             : m_light(src.m_light), 
               m_attributes(src.m_attributes)
{
}

EX_ONX_Model_RenderLight& EX_ONX_Model_RenderLight::operator=(const EX_ONX_Model_RenderLight& src)
{
  if ( this != &src )
  {
    m_light = src.m_light;
...
EX_ONX_Model_Object::EX_ONX_Model_Object() 
                 : m_bDeleteObject(0),
                   m_object(0), 
                   m_ref_count(0)
{
...
void EX_ONX_Model_Object::Destroy()
{
  if ( m_ref_count ) 
  {
    if ( *m_ref_count > 0 )
...
EX_ONX_Model_Object::EX_ONX_Model_Object(const EX_ONX_Model_Object& src) 
             : m_bDeleteObject(0),
               m_object(0),
               m_ref_count(0)
{
...
EX_ONX_Model_Object& EX_ONX_Model_Object::operator=(const EX_ONX_Model_Object& src)
{
  if ( this != &src )
  {
    Destroy();
    ...
    if ( 0 != m_object && m_bDeleteObject ) 
    {
      ...
      else 
      {
        ...
        *m_ref_count = 2; // 2 because this and src reference same m_object
        const_cast<EX_ONX_Model_Object*>(&src)->m_ref_count = m_ref_count; // need to defeat const
      }
    }
  }
  return *this;
...
EX_ONX_Model_UserData::EX_ONX_Model_UserData() : m_uuid(ON_nil_uuid)
{
}

EX_ONX_Model_UserData::~EX_ONX_Model_UserData()
{
}

EX_ONX_Model_UserData::EX_ONX_Model_UserData(const EX_ONX_Model_UserData& src) 
             : m_uuid(src.m_uuid),
               m_goo(m_goo)
...
EX_ONX_Model_UserData& EX_ONX_Model_UserData::operator=(const EX_ONX_Model_UserData& src)
{
  if ( this != &src )
  {
    m_goo = src.m_goo;
...
void EX_ONX_Model::DestroyCache()
{
...
ON_BoundingBox EX_ONX_Model::BoundingBox() const
{
  if( !m__object_table_bbox.IsValid() && m_object_table.Count() > 0 )
  {
    ...
    for ( i = 0; i < object_count; i++ )
    {
      ...
      const_cast<EX_ONX_Model*>(this)->m__object_table_bbox = bbox;
    }
  }
  return m__object_table_bbox;
}

void EX_ONX_Model::GetRenderMaterial( const ON_3dmObjectAttributes& attributes, ON_Material& material ) const
{
...
void EX_ONX_Model::GetRenderMaterial( 
      int object_index,
      ON_Material& material 
      ) const
{
...
void EX_ONX_Model::GetLinetype( const ON_3dmObjectAttributes& attributes, ON_Linetype& linetype ) const
{
...
ON_Color EX_ONX_Model::WireframeColor( const ON_3dmObjectAttributes& attributes ) const
{
...
ON_Color EX_ONX_Model::WireframeColor(int object_index) const
{
  ON_Color c;
  if ( object_index < 0 || object_index >= m_object_table.Count() )
  {
...
void EX_ONX_DumpView( ON_TextLog& dump, const ON_3dmView& view )
{
  view.Dump(dump);
}

void EX_ONX_Model::DumpSummary( ON_TextLog& dump ) const
{
...
void EX_ONX_Model::DumpBitmapTable( ON_TextLog& dump) const
{
  int i;
  for ( i = 0; i < m_bitmap_table.Count(); i++ )
  {
...
void EX_ONX_Model::DumpTextureMappingTable( ON_TextLog& dump) const
{
  int i;
...
void EX_ONX_Model::DumpMaterialTable( ON_TextLog& dump) const
{
  int i;
...
void EX_ONX_Model::DumpLinetypeTable( ON_TextLog& dump ) const
{
  int i;
...
void EX_ONX_Model::DumpLayerTable( ON_TextLog& dump) const
{
  int i;
...
void EX_ONX_Model::DumpLightTable( ON_TextLog& dump) const
{
  int i;
...
void EX_ONX_Model::DumpGroupTable( ON_TextLog& dump) const
{
  int i;
...
void EX_ONX_Model::DumpFontTable( ON_TextLog& dump) const
{
  int i;
...
void EX_ONX_Model::DumpDimStyleTable( ON_TextLog& dump) const
{
  int i;
...
void EX_ONX_Model::DumpHatchPatternTable( ON_TextLog& dump) const
{
  int i;
...
void EX_ONX_Model::DumpIDefTable( ON_TextLog& dump) const
{
  int i;
...
void EX_ONX_Model_Object::Dump( ON_TextLog& dump ) const
{
  if ( 0 != m_object )
  {
    m_object->Dump(dump);
...
void EX_ONX_Model::DumpObjectTable( ON_TextLog& dump) const
{
  int i;
...
void EX_ONX_Model::DumpHistoryRecordTable( ON_TextLog& dump) const
{
  int i;
  for ( i = 0; i < m_history_record_table.Count(); i++ )
  {
...
void EX_ONX_Model::DumpUserDataTable( ON_TextLog& dump) const
{
  int i;
  for ( i = 0; i < m_userdata_table.Count(); i++ )
  {
    const EX_ONX_Model_UserData& ud = m_userdata_table[i];
...
void EX_ONX_Model::Dump( ON_TextLog& dump ) const
{
...
{
public:
  ON__CIndexMaps( EX_ONX_Model& model ) 
    : m_model(model),
  ...
  */
  int RemapGeometryAndObjectAttributes( EX_ONX_Model_Object& );
...
int ON__CIndexMaps::CreateHelper()
{
  ...
  if ( m_model.m_layer_table.Count() < 1 )
  {
    ...
    if ( !EX_ONX_IsValidName(layer.m_name) )
  ...
  if ( m_model.m_font_table.Count() < 1 )
  {
    ON_Font font;
    font.Defaults();
    if ( !EX_ONX_IsValidName(font.m_font_name) )
  ...
  if ( m_model.m_dimstyle_table.Count() < 1 )
  {
    ...
    if ( !EX_ONX_IsValidName(dimstyle.m_dimstyle_name) )
  ...
  {
    EX_ONX_Model_RenderLight& light = m_model.m_light_table[i];
  ...
  {
    EX_ONX_Model_Object& mo = m_model.m_object_table[i];
...
int ON__CIndexMaps::RemapGeometryAndObjectAttributes( EX_ONX_Model_Object& model_object )
{
       ...
       )
{
  ...
  InspectProperties (m_properties);
  ...
  if ( archive.BeginRead3dmLightTable() )
  {
    ...
    for( count = 0; true; count++ ) 
    {
      ...
      EX_ONX_Model_RenderLight& light = m_light_table.AppendNew();
      ON_UserDataHolder ud;
...
//    int object_filter = 0; 
    ...
    for( count = 0; true; count++ ) 
    {
      ...
      rc = archive.Read3dmObject(&pObject,&attributes,0);
      if ( rc == 0 )
      ...
      if ( pObject ) 
      {
        int rx = ShouldKeepObject (pObject, attributes);
        if (rx > 0) {
          EX_ONX_Model_Object& mo = m_object_table.AppendNew();
          mo.m_object = pObject;
          ...
          delete pObject;
          if (rx < 0)
            return false;       // stop reading RIGHT NOW!
        }
      }
      else
      {
  ...
  for(count=0;true;count++)
  {
    ...
    EX_ONX_Model_UserData& ud = m_userdata_table.AppendNew();
    ud.m_uuid = uuid;
...
static 
void EX_ONX_Model_WriteHelper(ON_BinaryFile& file)
{
       ...
       )
{
  ...
  if ( !IsValid(error_log) )
  {
    ...
    if ( error_log) error_log->Print("EX_ONX_Model::Write Your model is not valid and will not be saved.\n");
    return false;
  }
  ...
  if ( version < 2 || version > 4 )
  {
    ...
    version = 4;
    if ( error_log) error_log->Print("EX_ONX_Model::Write version parameter = %d; it must be 2, 3, or 4.\n",
                    version);
  }
  ...
  if ( !archive.WriteMode() )
  {
    ...
    // archive constructor.
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Mode() is not ON::write3dm.\n"
                    "See EX_ONX_Model::Write example in the header file.\n");
    return false;
  }
  ...
  if ( !ok )
  {
    ...
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmStartSection() failed.\n"
  ...
  if ( !ok )
  {
    ...
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmProperties() failed.\n"
  ...
  if ( !ok )
  {
    ...
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmSettings() failed.\n"
  ...
  if ( !ok )
  {
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmBitmapTable() failed.\n");
    return false;
  }
  for( i = 0; ok && i < m_bitmap_table.Count(); i++ )
  {
    ...
    if ( !ok )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmBitmap(m_bitmap_table[%d]) failed.\n",i);
    }
  }
  if ( !archive.EndWrite3dmBitmapTable() )
  {
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmBitmapTable() failed.\n");
    return false;
  }
  if (!ok)
    return false;
  ...
  if ( archive.Archive3dmVersion() >= 4 )
  {
    ...
    if ( !ok )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmTextureMappingTable() failed.\n");
      return false;
    }
    for( i = 0; ok && i < m_mapping_table.Count(); i++ )
    {
      ...
      if ( !ok )
      {
        if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmTextureMapping(m_mapping_table[%d]) failed.\n",i);
      }
    }
    if ( !archive.EndWrite3dmTextureMappingTable() )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmTextureMappingTable() failed.\n");
      return false;
    }
    if (!ok)
      return false;
  ...
  if ( !ok )
  {
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmMaterialTable() failed.\n");
    return false;
  }
  for( i = 0; ok && i < m_material_table.Count(); i++ )
  {
    ...
    if ( !ok )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmMaterial(m_material_table[%d]) failed.\n",i);
    }
  }
  if ( !archive.EndWrite3dmMaterialTable() )
  {
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmMaterialTable() failed.\n");
    return false;
  }
  if (!ok)
    return false;
  ...
  if ( archive.Archive3dmVersion() >= 4 )
  {
    ...
    if ( !ok )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmLinetypeTable() failed.\n");
      return false;
    }
    for( i = 0; ok && i < m_linetype_table.Count(); i++ )
    {
      ...
      if ( !ok )
      {
        if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmLinetype(m_linetype_table[%d]) failed.\n",i);
      }
    }
    if ( !archive.EndWrite3dmLinetypeTable() )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmLinetypeTable() failed.\n");
      return false;
    }
    if (!ok)
      return false;
  ...
  {
    // make sure m_settings is valid
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmLayerTable() failed.\n");
    return false;
  }
  for( i = 0; ok && i < m_layer_table.Count(); i++ )
  {
    ...
    if ( !ok )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmLayer(m_layer_table[%d]) failed.\n",i);
    }
  }
  if ( !archive.EndWrite3dmLayerTable() )
  {
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmLayerTable() failed.\n");
    return false;
  }
  if (!ok)
    return false;
  ...
  {
    // make sure m_settings is valid
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmGroupTable() failed.\n");
    return false;
  }
  for( i = 0; ok && i < m_group_table.Count(); i++ )
  {
    ...
    if ( !ok )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmGroup(m_group_table[%d]) failed.\n",i);
    }
  }
  if ( !archive.EndWrite3dmGroupTable() )
  {
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmGroupTable() failed.\n");
    return false;
  }
  if (!ok)
    return false;
  ...
  if ( archive.Archive3dmVersion() >= 3 )
  {
    ...
    {
      // make sure m_settings is valid
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmFontTable() failed.\n");
      return false;
    }
    for( i = 0; ok && i < m_font_table.Count(); i++ )
    {
      ...
      if ( !ok )
      {
        if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmFont(m_font_table[%d]) failed.\n",i);
      }
    }
    if ( !archive.EndWrite3dmFontTable() )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmFontTable() failed.\n");
      return false;
    }
    if (!ok)
      return false;
  ...
  if ( archive.Archive3dmVersion() >= 3 )
  {
    ...
    {
      // make sure m_settings is valid
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmDimStyleTable() failed.\n");
      return false;
    }
    for( i = 0; ok && i < m_dimstyle_table.Count(); i++ )
    {
      ...
      if ( !ok )
      {
        if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmDimStyle(m_dimstyle_table[%d]) failed.\n",i);
      }
    }
    if ( !archive.EndWrite3dmDimStyleTable() )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmDimStyleTable() failed.\n");
      return false;
    }
    if (!ok)
      return false;
  ...
  {
    // make sure m_settings is valid
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmLightTable() failed.\n");
    return false;
  }
  for( i = 0; ok && i < m_light_table.Count(); i++ )
  {
    ...
    if ( !ok )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmLight(m_light_table[%d]) failed.\n",i);
    }
  }
  if ( !archive.EndWrite3dmLightTable() )
  {
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmLightTable() failed.\n");
    return false;
  }
  if (!ok)
    return false;
  ...
  if ( archive.Archive3dmVersion() >= 4 )
  {
    ...
    if ( !ok )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmHatchPatternTable() failed.\n");
      return false;
    }
    for( i = 0; ok && i < m_hatch_pattern_table.Count(); i++ )
    {
      ...
      if ( !ok )
      {
        if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmHatchPattern(m_hatch_pattern_table[%d]) failed.\n",i);
      }
    }
    if ( !archive.EndWrite3dmHatchPatternTable() )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmHatchPatternTable() failed.\n");
      return false;
    }
    if (!ok)
      return false;
  ...
  if ( archive.Archive3dmVersion() >= 3 )
  {
    ...
    {
      // make sure m_settings is valid
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmInstanceDefinitionTable() failed.\n");
      return false;
    }
    for( i = 0; ok && i < m_idef_table.Count(); i++ )
    {
      ...
      if ( !ok )
      {
        if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmInstanceDefinition(m_IDef_table[%d]) failed.\n",i);
      }
    }
    if ( !archive.EndWrite3dmInstanceDefinitionTable() )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmInstanceDefinitionTable() failed.\n");
      return false;
    }
    if (!ok)
      return false;
  ...
  if ( !ok )
  {
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmObjectTable() failed.\n");
    return false;
  }
  for( i = 0; ok && i < m_object_table.Count(); i++ )
  {
    if ( 0 != m_object_table[i].m_object )
    {
      ...
      if ( !ok )
      {
        if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmObject(m_IDef_table[%d]) failed.\n",i);
      }
    }
  }
  if ( !archive.EndWrite3dmObjectTable() )
  {
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmObjectTable() failed.\n");
    return false;
  }
  if (!ok)
    return false;
  ...
  if ( archive.Archive3dmVersion() >= 4 )
  {
    ...
    if ( !ok )
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmHistoryRecordTable() failed.\n");
      return false;
    }
    ...
    {
      if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmHistoryTable() failed.\n");
      return false;
    }
    if (!ok)
      return false;
  ...
  for( i = 0; ok && i < m_userdata_table.Count(); i++ )
  {
    ...
    if ( ON_UuidIsNotNil(ud.m_uuid) )
    {
      ...
      {
        // make sure m_settings is valid
        if ( error_log) error_log->Print("EX_ONX_Model::Write archive.BeginWrite3dmUserTable(m_userdata_table[%d].m_uuid) failed.\n",i);
        return false;
      }
      ok = archive.Write3dmAnonymousUserTable(ud.m_goo);
      if ( !ok )
      {
        if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmAnonymousUserTable(m_userdata_table[%d].m_goo) failed.\n",i);
      }
      if ( !archive.EndWrite3dmUserTable() )
      {
        if ( error_log) error_log->Print("EX_ONX_Model::Write archive.EndWrite3dmUserTable() failed on m_userdata_table[&d].\n",i);
        ok = false;
      }
    }
  }
  ...
  {
    ok = false;
    if ( error_log) error_log->Print("EX_ONX_Model::Write archive.Write3dmEndMark() failed.\n");
  }

  return ok;
}

bool EX_ONX_Model::IsValid( ON_TextLog* text_log ) const
{
  // Audit with no repairs will simply complain if it
  // finds something wrong;
  int i = const_cast<EX_ONX_Model*>(this)->Audit(false,NULL,text_log,NULL);
  return (i>=0);
}

int EX_ONX_Model::ObjectIndex( ON_UUID object_uuid ) const
{
...
int EX_ONX_Model::IDefIndex( ON_UUID idef_uuid ) const
{
  // In a high quality app, you may want to override this
  // and do something a little smarter.
...
int EX_ONX_Model::IDefIndex( const wchar_t* idef_name ) const
{
...
void EX_ONX_Model::GetUnusedIDefName( ON_wString& idef_name ) const
{
  int i = 1;
  for(i = 1; i < 100000; i++ )
  {
        ...
        ) const
{
  ...
  for ( j = 0; j < iref_idef.m_object_uuid.Count(); j++ )
  {
    ...
    if ( obj_index < 0 )
      continue;
    const EX_ONX_Model_Object& obj = m_object_table[obj_index];
    if ( 0 == obj.m_object )
      continue;
...
int EX_ONX_Model::LayerIndex( const wchar_t* layer_name ) const
{
  // slow and stupid search - what do you expect for free?
  // In a high quality app, you will want to override this
  // and do something a little smarter.
...
void EX_ONX_Model::GetUnusedLayerName( ON_wString& layer_name ) const
{
  int i = 1;
  for(i = 1; i < 100000; i++ )
  {
      ...
      )
{
  ...
  for ( i = 0; i < count; i++ )
  {
    EX_ONX_Model_RenderLight& xlight = model.m_light_table[i];
      ...
      )
{
  ...
  for ( i = 0; i < count; i++ )
  {
    ...
    if ( !EX_ONX_IsValidName(layer_name) )
    {
      if ( text_log )
      {
  ...
  if ( count > 0 )
  {
    ...
    for ( i = 0; i < count; i++ )
    {
      ...
      if ( ON_nil_uuid == id_list[i].m_id )
      {
        ...
        {
          uuid = ON_nil_uuid;
          if ( ON_CreateUuid(uuid) && !ON_UuidIsNil(uuid) )
          {
            id_list[i].m_id = uuid;
            rep_count++;
            if ( text_log )
      ...
      )
{
  ...
  if ( count > 0 )
  {
    ...
    for ( i = 0; i < count; i++ )
    {
      EX_ONX_Model_RenderLight& light = model.m_light_table[i];
    ...
    rc = AuditIdsHelper(
              id_list,
              0, // no EX_ONX_Model id index for lights
              bAttemptRepair,
              repair_count,
              text_log,
    ...
    if ( rc && bAttemptRepair )
    {
      for ( i = 0; i < count; i++ )
      {
        EX_ONX_Model_RenderLight& light = model.m_light_table[id_list[i].m_i];
      ...
      )
{
  ...
  for ( i = 0; i < count; i++ )
  {
    ...
    if ( !EX_ONX_IsValidName(idef_name) )
    {
      if ( text_log )
    ...
    if ( bAttemptRepair )
    {
      ...
      if ( EX_ONX_IsValidName(new_name) && -1 == model.IDefIndex(new_name) )
      {
  ...
  for ( i = 0; i < count; i++ )
  {
    ...
    for ( j = k = 0; j < idef.m_object_uuid.Count(); k++ )
    {
      ...
      {
        EX_ONX_Model_Object& obj = model.m_object_table[obj_index];
      ...
      )
{
  ...
  for ( i = 0; i < count; i++ )
  {
    EX_ONX_Model_Object& obj = model.m_object_table[i];
    if ( 0 == obj.m_object )
    {
      rc = 7;
      if ( text_log )
